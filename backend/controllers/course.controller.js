...
export const updateCourse = async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ error: "Error in course uploading" });
...
export const deleteCourse = async (req, res) => {
    ...
    try {
        ...
        if (!course) {
            return res.status(404).json({ errors: "can't delete creatred by other admin" })
        }
...
export const buyCourses = async (req, res) => {
    ...
    try {
        ...
        const paymentIntent = await stripe.paymentIntents.create({
            amount: amount,
            currency: "inr",
            description: 'SaaS subscription - Plan Gold - June 2025',
